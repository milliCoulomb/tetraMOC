...
public:
    ...
    RayTracer(const MeshHandler& mesh_handler, const Vector3D& fixed_direction, const double direction_weight, size_t direction_index);
    ...
    // Getter for the direction index (only relevant in CONSTANT_DIRECTION mode)
    size_t getDirectionIndex() const {
        assert(mode_ == RayTracerMode::CONSTANT_DIRECTION && "getDirectionIndex() called in VARIABLE_DIRECTION mode");
